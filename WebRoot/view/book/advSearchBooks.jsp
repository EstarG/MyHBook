...
<s:form action="advSearch_BookAction" method="post">
	
	<!-- 高级搜索框里面的内容带过来 -->
	<s:hidden name="queryBookInfo.name" value="%{ queryBookInfo.name }"></s:hidden>
	<s:hidden name="queryBookInfo.authorName" value="%{ queryBookInfo.authorName }"></s:hidden>
	<s:hidden name="queryBookInfo.publisher" value="%{ queryBookInfo.publisher }"></s:hidden>
	<s:hidden name="queryBookInfo.isbn" value="%{ queryBookInfo.isbn }"></s:hidden>
	<s:hidden name="queryBookInfo.beginDate" value="%{ queryBookInfo.beginDate }"></s:hidden>
	<s:hidden name="queryBookInfo.endDate"  value="%{ queryBookInfo.endDate }"></s:hidden>
			...
			<s:iterator var="newBook" value="books">
	        	<div class="item" sku="1055640717">
	                ...
	                <dd class="p-price">
	                    ...
	                    <div class="dd">
	                    	<strong>￥<fmt:formatNumber type="number" value="${ newBook.hbPrice}" maxFractionDigits="1"/></strong><span class="discount">(${ newBook.discount }折)</span> 
	                    </div>
	                </dd>
	                <dd class="p-market">
	            ...
	            <ul class="summary">            
	                <li class="summary-author">
	                    ...
	                    <div class="dd">
	           				<s:iterator var="author" value="#newBook.TAuthors" status="vs">  
	           					<c:if test="${vs['index'] == 0}">
									 <a target="_blank" href="search_BookAction?queryBookInfo.clientInputInfo=${ author.name }" target="_blank"><s:property value="#author.name"/>  </a>
								</c:if >
								<c:if test="${vs['index'] != 0}">
								　| <a target="_blank" href="search_BookAction?queryBookInfo.clientInputInfo=${ author.name }" target="_blank"><s:property value="#author.name"/>  </a>
								</c:if>
